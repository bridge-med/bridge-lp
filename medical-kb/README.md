...
| 診療報酬項目(items・evidence付き) | **34件**(重点5科のシナリオ解決セット)。材料・薬剤・体制加算は未了(issues #10) |
| 施設基準 | **8件**(運動器リハI/II/III・生活習慣病管理料・人工腎臓・導入期加算・透析液水質・在支診)。人員・設備・様式番号まで通知原文引用付き |
| 併算定ルール(curated) | **8件**(外来管理加算×処置リハ、生活習慣病(I)包括・(II)包括・(I)→(II)6月制限、屈折×矯正視力、透析薬剤包括、透析検査包括、訪問診療日の再診往診不可)。全て原文引用付き |
| 診療シナリオ | **重点5科すべて算定スロット解決済み**(点数・除外理由・根拠付き。材料等の一部スロットのみ未解決) |
| Knowledge Pack | **重点5科すべてに[確定]記載を反映**(収益構造の実点数試算まで) |
