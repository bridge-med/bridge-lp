...
6. quote(原文抜粋)は原典テキストの行を機械コピーする(手打ちしない・v55〜v57の教訓)。その際の手順:
   (a) 拾った行の直前にある区分番号の見出し行(例「Ｃ００２ 在宅時医学総合管理料」)を併記して、別区分の同名条文
   (例: C003の在宅医療情報連携加算)を取り込んでいないか確認する (b) 行末が句読点でも助詞でもない行(表・箇条)は
   1空白を入れて詰める。多段組の様式(届出用紙)は行順取り込みで列が混線するため機械コピーの対象外とし、
   様式の文言はrule.quoteに入れず evidence.json(field=condition・note)に置く
   (c) 行またぎで半角英数字に接する箇所(「様式\n42」)は1空白を入れて連結し、通知の「様式 42」の書式を保つ。号・行の見出し
   (ア イ (１) ① や表の行番号)の継ぎ目も1空白 (d) 引用は原典の一箇所から連続して取る。号(1)(2)…をまたいで抜粋しない=間の
   なお書きごと写すか、末尾を無標で切る(v61 便Z'の教訓: 「…」29本(一意28)のうち（中略）で済んだものは0本)。末尾で切るときは、
   柱書(「以下のいずれかを満たすこと。」等)を最後の一文にしない=示すと予告した列挙を1つも含まない切り方は採らない
