...
// v3(2026-09-26): 人を主語にした音(名前が落ちる音・現場ごとの音程・肩書きの帯の音)は置かない。主語は見る人の「いま」
// 仕上げ: 残響 → 低域モノ → -14 LUFS・wav の真のピーク -1.5 dBTP 以下(書き出した mp4 で -1 dBTP 未満。天井に触れるのは最後の着地だけ)→ 最後の 50 ms を閉じる
// ================================================================
...
const TARGET_LUFS = -14, TP_MAX = -1.5;                       // 積分ラウドネスと、wav の真のピークの上限(dBTP)。書き出した mp4(AAC 256k)のはみ出し(実測 +0.12〜0.18 dB)を見込んで -1.5 にし、mp4 で -1 dBTP 未満に収める
...
/* 音程の曲がり: t0→t1 で m0 → m1(expoInOut。画の屈曲は sineInOut で、区間は同じ 12.773–13.008)、t2 まで保って rel で消える */
...
function score() {
  ...
  // --- 小節 1 · いま
  impact(0, 0.60, [[len => boomWave(len, D1, 0.35), 1]], { dur: 0.37, room: 0, hall: 0, duckDepth: 0 });   // 原点の点: サブの一打 D1
  click(0, 0.45, { room: 0 });                                                                            // 乾いたクリック
  whoosh(G(1, 0.5), 0.55 - G(1, 0.5), 0.25, 800, 5000, 0, 0.6, 0.7);       // ペン先が走る(中央 → 右)
  tick(G(1, 0.75), 0.18, 2400, scr(922));                                   // 「いま」が線から立つ(い 0.319・ま 0.370 のあいだ)。0.352–1.172 は画も音も静かな間
  tick(G(1, 2.5), 0.10, 1800, scr(1500));                                   // ペンがまた動き出す
  whoosh(G(1, 3), BEAT, 0.45, 300, 6000, 0.6, -0.6, 0.5);                  // カメラのウィップ(右 → 左、山は 1.641)
  // --- 小節 2 · 一本しか見えていなかった道(地の拍だけ。砂色の分かれ目の点に D6 のマレット)
  ...
  tick(G(3), 0.12, 520, 0, { f1: 380, dur: 0.08, dec: 70, room: 0.1 });    // 低く、くぐもったティック(一文が線へ沈む)
  click(G(3, 0.75), 0.20, { pan: scr(509) });                              // 可能性に気づく
  ...
  // --- 小節 6 · 型(評価 / 理解 / 設計 / 実践 / 再評価)。語が線から上がる瞬間に 1 語 1 音、強さ 0.24。
  ...
  swell(G(6, 3.5), G(7), 0.30, [74, 77, 81]);                              // 原則への入り
  // --- 小節 7 · THE BREATH, THE BEND(キックなし。サブと和音だけ)
  air(G(7), 1.4, 0.20, { rise: 0.46875 });                                  // 原則が線から立つ空気
  ...
  glide(G(7, 3.25), G(7, 3.75), G(8), 69, 74, 0.12);                       // 線の持ち上がり(12.773–13.008)と一緒に 4 度上がる(A4 → D5)
  ...
  air(G(8, 0.5), 0.6, 0.08, { rise: 0.234375, hall: 0.2 });                // 字間が詰まる。これより後に新しい音はない
}
score();
