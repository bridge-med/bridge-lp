...
| ページ | 役割 | 想定滞在 | 主な次の一歩 |
|---|---|---|---|
| Home | 最初の画面で誰に何が役立つかを伝え、道具への入口(おすすめ3点)を渡す。場面1の主CTAは Compass(最初に体験できる入口)。ヒーローはスクロールで場面が変わる固定ステージ(明るい場のガラスの立体=事前レンダ `images/hero/`・差し替え可(docs/hero-asset-spec.md) → 夜の場でゲームの実画面が主役)。非表示の場面は inert で操作対象から外す。動きを減らす設定・JS なしでは場面1だけの静止した1画面 | 30秒〜1分 | Compass / 個別プロダクト / Products / Philosophy / About / note |
