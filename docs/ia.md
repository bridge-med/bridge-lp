...
```
/                       Home           … 玄関。誰に何が役立つかを最初の画面(見出し2行+一文)で伝え、「試しているもの」(作業机のように、大きさの違う入口)から道具へ渡す。用途別の索引は Products に任せる
...
├── products/           Products       … プロダクトカタログ(用途別・全29点)
...
1. **Homeは玄関、長い説明はしない** — Homeの役割は「誰に・何が役立つか」の一言と、次に触るもの(試しているもの)を渡すこと。理念の全文はPhilosophyに、用途別の索引はProductsに置く。(v2.1: 「説明はしない」から改定。初めて訪れた医療職が最初の画面で用途を理解できることを優先する / v2.2: 常時表示の文字量を半分以下にし、見て分かる構成に。用途別索引をHomeから外す / v2.6: 商品の一覧に見せない。Home=気になるものを触る、Products=全部探す)
...
| ページ | 役割 | 想定滞在 | 主な次の一歩 |
|---|---|---|---|
| Home | 最初の画面で誰に何が役立つかを伝え、道具への入口(試しているもの: Compass=大きな実画面 / Clinic Town 3D=文字の行 / 小さな道具3つ / 止めているもの)を渡し、本人の短い文章から考え方へつなぐ。場面1の主CTAは Compass(最初に体験できる入口)。ヒーローはスクロールで場面が変わる固定ステージ(明るい場のガラスの立体=事前レンダ `images/hero/`・差し替え可(docs/hero-asset-spec.md) → 夜の場でゲームの実画面が主役)。非表示の場面は inert で操作対象から外す。動きを減らす設定・JS なしでは場面1だけの静止した1画面 | 30秒〜1分 | Compass / 個別プロダクト / Products / Philosophy / About / Community / note |
...
│ 試しているもの  │    │ Manifesto(夜) │   │ Footer(地図)   │
│ 本人の言葉+入口 │    │ Where to next  │   └──────────────┘
│ Footer(地図)   │    │ Footer         │
└──────────────┘    └──────────────┘
```
...
`docs/design-system.md` §3 に移した(2026-09-22)。部品の用途・状態・増やすときの規則は同書が唯一の台帳。ここに複製しない。
...
- **プロダクト** … `products/index.html`の`PRODUCTS`配列に1オブジェクト追加するだけ(`use`=用途・`t`=日本語の機能名・`cta`=ボタン文言・`facts`=確認済みの利用条件)。用途は`USES`配列で管理し、用途ごとのセクションは自動生成。トップの「ここでは、いろいろ試しています」は静的HTMLなので、並べるものを入れ替えるとき・件数(29)が変わるときは`index.html`も直す(v2.2: トップの用途別索引は廃止)。
...
### 変更履歴
- v2.6 (2026-09-23): Homeの「おすすめ3つ」(カード)と独立した「考え方」セクションを外す。「ここでは、いろいろ試しています」(Compass の実画面と翻訳の2行 / 直し続けている Clinic Town 3D / 小さく試しているもの / 止めているもの / Productsへの文字リンク)→ 本人の言葉と明朝の一文 → 入口(考え方・運営者・話をする・note)に。Heroの文言をサービス説明から話し言葉へ。
- v2.5 (2026-09-22): 6章の部品一覧を docs/design-system.md §3 へ移し、二重管理をやめた。
...
- v2.3 (2026-09-23): Homeの入口を BRIDGE Compass に。場面1の主CTAを「3分で可能性を探す」(compass/)に変え、おすすめ3点を Compass / Clinic Town 3D / 経験の棚卸し に。場面2(Clinic Town 3D)は維持。
