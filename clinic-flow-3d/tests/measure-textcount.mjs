...
// v81 便AJ-2: 静的な説明文だけのセレクタ集計(遊ぶ既定の閾値 ≤200 はこの数え方)
const STATIC = '.card-title small, .ctrl-note, .ctrl-head small, .shop-hint, .jihi-stat';
const PORT = process.env.PORT || '8767';
const LANE = process.env.LANE === 'learn' ? 'learn' : 'play'; // 扉で選ぶレーン
async function measure(page, tab) {
  ...
  return page.evaluate(([EXPL, VOICE, STATIC]) => {
    ...
    let stat = 0;
    for (const el of pane.querySelectorAll(STATIC)) {
      if (!vis(el)) continue;
      stat += el.textContent.replace(/\s+/g, '').length;
    }
    return { total, first, stat, sum: total.a + total.b + total.c + total.d, firstSum: first.a + first.b + first.c + first.d, height: document.documentElement.scrollHeight };
  }, [EXPL, VOICE, STATIC]);
}
for (const w of [375, 390]) {
  for (const stage of ['day1', 'day10']) {
    ...
    await page.goto(`http://localhost:${PORT}/clinic-flow-3d/index.html`, { waitUntil: 'domcontentloaded' });
    await page.evaluate(() => localStorage.clear());
    await page.reload({ waitUntil: 'domcontentloaded' }); await page.waitForTimeout(300);
    await page.evaluate((ln) => { document.querySelector('[data-gate="orthopedics"]')?.click(); document.querySelector(`[data-lane="${ln}"]`)?.click(); document.getElementById('gateGo')?.click(); document.getElementById('tutSkip')?.click(); }, LANE);
    ...
    console.log(w, stage, LANE, JSON.stringify(out));
    await page.close();
  }
}
await browser.close();
