 ...
 * 名簿を比3.5以上まで積む(区分3への落下)。測るもの: 時間外加算・濾過加算の件数と算定額、区分3セッション数、売上総額。
...
function run(seed, cfg) {
  ...
  const rand = rng(seed);
  let n1 = 0, n1Yen = 0, n13 = 0, n13Yen = 0, hd3 = 0, hd1 = 0, revenue = 0;
  ...
  return { seed, n1, n1Yen, n13, n13Yen, hd1, hd3, revenue, census: dept.pt.length, kubun1: dept.fs.includes('r08-fs-j038-1') };
}
...
for (const [label, cfg] of CFGS) {
  ...
  console.log(`\n# ${label}`);
  for (const r of res) console.log(`  seed=${r.seed} 人工腎臓 区分1 ${r.hd1}件/区分3 ${r.hd3}件 / 時間外 ${r.n1}件 ${r.n1Yen.toLocaleString()}円 / 濾過 ${r.n13}件 ${r.n13Yen.toLocaleString()}円 / 売上 ${r.revenue.toLocaleString()}円 / 患者${r.census}人${r.kubun1 ? '' : '・区分1を外れた'}`);
  console.log(`  中央値: 時間外 ${med(res.map((r) => r.n1Yen)).toLocaleString()}円 / 濾過 ${med(res.map((r) => r.n13Yen)).toLocaleString()}円 / 区分3 ${med(res.map((r) => r.hd3))}件 / 売上 ${med(res.map((r) => r.revenue)).toLocaleString()}円 (${DAYS}日)`);
  const y = (v) => Math.round(v * 365 / DAYS / 10000);
  console.log(`  年換算: 時間外 約${y(med(res.map((r) => r.n1Yen)))}万円・濾過 約${y(med(res.map((r) => r.n13Yen)))}万円 (このシナリオの値。点推定ではない)`);
}
