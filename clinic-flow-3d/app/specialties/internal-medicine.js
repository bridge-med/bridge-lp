  ...
  const M = {
    ...
    main: {
      ...
      preset: {
        ...
        jihiHide: ['selfReha', 'prpOn', 'goods'], // 自費リハ延長・PRP療法・物販(サポーター等)は運動器の自費(v76 保留#43・v82/v83 保留#45)
        // キホン集(TEXTBOOK)の科別差し替え(v77 保留#42・v83 保留#47 ⑮⑰・editor 原稿)。整形前提の③⑥⑨⑩⑫だけ。制度上の数値は書かない
        textbook: {
          2: { t: '③ 単価は複合で作る', b: '内科の単価は初再診料+生活習慣病管理料+検体検査+処方の組み合わせ。管理料の方針(I/II)が検査の範囲まで決める。' },
          5: { t: '⑥ 継続管理はLTVで考える', b: '高血圧や糖尿病の通院は年単位で続く。1回ごとの単価より通院回数の総和が売上を決める。中断を減らすことが最大の改善になる。' },
          9: { t: '⑨ 施設基準は経営の土台', b: '内科の施設基準は生活習慣病管理料の院内掲示や、外来医療等調査への参加とデータ提出体制の届出。設備でなく体制と記録の継続が算定の土台になる。' },
          10: { t: '⑩ 分院は専従の壁', b: '分院の体制要件は施設ごとに満たす必要がある。本院の看護師を分院に兼務させることはできない。分院展開のボトルネックは資金より採用になる。' },
          12: { t: '⑫ 設備投資は回収期間で決める', b: '検体検査を院内でやるか外注するか、自動精算機を入れるか。判断基準は「欲しい」ではなく、投資が何日の診療で回収できるかという期間になる。' },
          15: { t: '⑮ 自費は価値設計から', b: 'AGAは「保険でできないこと」への対価。価格は価値で決める。評判が低いうちは売れない — 保険診療の信頼が自費の土台。' },
          17: { t: '⑰ 客層(セグメント)から逆算する', b: '高齢者=高血圧・糖尿病の継続管理。勤労者=健診がきっかけ。病院・ケアマネ・老健ごとに来る客層は違う — 営業先から選ぶ。' },
        }, // 整形専用の採用・設備は出さない(第14条=ロック行にもしない)。運動器エコー・DEXA・MRIは整形本院の経路でしか算定されない(v74)
        // 営業先の文言(整形はリハ紹介前提)。効果の数値は同じ=文言だけ(v67・editor照合対象)
        rel: {
          hospital: { effect: '紹介患者 +Lv人/日(退院後の持病の管理)', desc: '地域連携室との関係。退院後の血圧・血糖の管理先になる。' },
          company: { desc: '従業員の定期健診。健診で引っかかった人の受診先になる。' },
