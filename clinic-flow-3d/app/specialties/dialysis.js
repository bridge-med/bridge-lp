 ...
 *    クールの割付は添字/同時床数(装置台数と看護師×4床の小さい方)で行う(v57で是正)。
...
(function (root) {
  ...
  const M = {
    ...
    runDay(dept, ctx, api, agg) {
      ...
      seen.forEach((p, i) => {
        api.countVisit();
        // クールは同時に回せる床(装置台数と看護師×4床の小さい方)の順に埋める(1クール目→2→3)。
        // 3クール目=午後5時以降の開始とみなし時間外・休日加算(ゲーム上の仮定。v57で同時床数を看護師で絞る=13g⑧の是正)
        const perCool = Math.max(1, Math.min(dept.equip.beds, (dept.staff.nurses || 0) * P.nursePerBedsCool));
        const evening = dept.policy.cools >= 3 && Math.floor(i / perCool) >= 2;
