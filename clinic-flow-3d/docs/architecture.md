...
- **制度上の事実とゲーム上の仮定を分離する**。KB由来の行(レシートで青)は令和8年度の実点数+出典。
  KB未登録の行為(物療等)は「概算」タグ付きで、順次KB登録して置き換える
...
本院候補は4科(開始の扉で選ぶ・v66〜v82)、法人の部門は別。本院候補以外の科は「部門」として法人タブから開設できる。
...
在宅部門(v37)は建物を持たず、街そのものが診療フィールドになる: 患者は14の地区
(戸建ての集まり12+マンション2棟(v50)。マンションは同一建物として在医総管の
単一建物人数セルをZAISOKAN(app/zaisokan.js)が選ぶ)に住み、期日の来た患者を
近い順に回るルートがタウンに描かれる(進んだ分は実線・残りは点線)。移動時間+診療時間が
1日の枠(480分/医師)を超えた分は翌日へ繰り越される — 移動効率がそのまま収益になる。
